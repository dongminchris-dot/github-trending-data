...
数据来自自建的 GitHub Trending 爬虫仓库，每天自动抓取并存储到 `data/{日期}.json`。

⚠️ 必须先确定数据仓库地址：让用户告知 GitHub 仓库名（格式 `owner/repo`），如果没有，提醒用户先完成部署。
...
已知数据仓库为 `{REPO_OWNER}/{REPO_NAME}`（请让用户确认后填入）。

使用 WebSearch 搜索：`site:github.com {REPO_OWNER}/{REPO_NAME} data/{昨天的日期}`
...
从结果中提取完整的 repositories 列表（通常 25 个），取 Top 10。

如果搜不到 JSON 内容，尝试：`{REPO_OWNER}/{REPO_NAME} trending top 10 {昨天日期}` 搜索 README 中可能包含的数据。如果 README 中也没有，用 WebSearch 搜索 `github trending repositories {昨天日期}` 作为兜底方案。
